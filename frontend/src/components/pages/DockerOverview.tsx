...
import { useStacks } from "@/hooks/useNewApi"; // Real-time unified stacks
import type { NavigationProps } from "@/types/navigation";
import { RealtimeStacksTest } from "../debug/RealTimeStackTest";
...
export const DockerOverviewPage: React.FC<DockerOverviewPageProps> = ({
  ...
  // Calculate quick stats from unified stack data
  const runningStacks = stacks.filter((s) => s.status === "running").length;
  const totalContainers = stacks.reduce(
    (total, stack) => total + (stack.containers?.total || 0),
    0
  );
  const runningContainers = stacks.reduce(
    (total, stack) => total + (stack.stats?.containers?.running || 0),
    0
  );
  ...
  const quickStats = [
    {
      ...
      icon: Container,
      color: "blue",
      loading: stacksLoading || !connected,
    },
    {
        ...
        <HStack justify="space-between" align="center">
          ...
          <HStack gap="2">
            <Box
              ...
              borderRadius="full"
              bg={
                connected ? "green.500" : stacksError ? "red.500" : "yellow.500"
              }
            />
